    ...
    if (typeof formFillBookmarklet !== 'undefined') {
        return;
            ...
            formFillBookmarklet = {
                ...
                init: function () {
                    if (typeof $ !== 'undefined') {
                        for (var i = 0, a = Object.keys(this.values); i < a.length; i++) {
                            prop = Object.keys(this.values)[i].toString();
                            ...
                            if ($('input[name*=' + prop + ']').eq(0).attr('type') === 'radio' || $('input[name*=' + prop + ']').eq(0).attr('type') === 'checkbox') {
                                doCases(prop, function (prop) {
                                    for (var b = 0, a = $('input[name*=' + prop + ']') ; b < a.length; b++) {
                                        $('input[name*=' + prop + '][value=' + formFillBookmarklet.values[prop] + ']:visible').prop('checked', true)
                                    }
                                });
                            }
                            if (prop.match('zip') || prop.match('post')) {
                                ...
                                doCases(prop, function (prop) {
                                    $('[name*=' + prop + ']:visible').val(formFillBookmarklet.values[Object.keys(formFillBookmarklet.values)[i]])
                                });
                            }
                        }
                    }
            ...
            $("head style").eq(0).append(".ffbtooltip {font-size: 15px; font-weight: bold; color: #000;position:relative} .ffbtooltip:before {content: '!'; position: absolute; top: -2px; border-radius: 30px; width: 18px; height: 18px; overflow: hidden; background: coral; font-weight: bold; color: white; line-height: 20px; text-align: center; left: -23px; font-size: 12px; box-shadow: 1px 1px 1px #aaa;}")
            var bookmarklet = "javascript: var version = 1, script = document.createElement('script'); script.type = 'text/javascript'; script.async = true; script.charset = 'UTF-8'; script.setAttribute('src', 'https://raw.github.com/Dathann/dnic/master/AutoFillbookmarklet.js' + '#' + Math.random()); document.documentElement.appendChild(script); script.onload = script.onreadystatechange = function () { var rs = script.readyState; if (!rs || rs === 'loaded' || rs === 'complete') { script.onload = script.onreadystatechange = null; if (version !== formFillBookmarklet.version) { alert('This bookmarklet is out of date!'); } } };"
            $("iframe").css({ border: "1px dashed red" }).before("<a href=\"" + bookmarklet + "\" class='ffbtooltip' title='Scripts cannot access frames with a different origin'>Paste this into an iframe</a>")
